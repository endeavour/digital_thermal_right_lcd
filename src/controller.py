...
class Controller:
    ...
    def load_layout(self):
        try:
            layout_path = os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "layout.json"
            )
    ...
    def get_device(self):
        try:
            print(
                f"Attempting to open HID device {hex(self.VENDOR_ID)}:{hex(self.PRODUCT_ID)}",
                flush=True,
            )
            device = hid.Device(self.VENDOR_ID, self.PRODUCT_ID)
            print("HID device opened successfully", flush=True)
            return device
        except Exception as e:
            print(f"Error initializing HID device: {e}", flush=True)
            return None
    ...
    def draw_speed_phantom_spirit(self, speed):
        ...
        if "speed_digits" in self.layout and len(self.layout["speed_digits"]) >= 4:
            ...
            if speed >= 10:
                self.draw_number(
                    (speed // 10) % 10, 1, [self.layout["speed_digits"][1]]
                )

            # Draw 100s digit if speed >= 100
            if speed >= 100:
                self.draw_number(
                    (speed // 100) % 10, 1, [self.layout["speed_digits"][2]]
                )
    ...
    def display_cpu_watts_mode(self):
        ...
        self.colors = self.get_config_colors(
            self.config, key=getattr(self, "color_mode", "usage"), metrics=metrics
        )
            ...
            if color.lower() == "random":
                ...
                if metric == "usage":
                    ...
                    if getattr(self, "display_mode", "cpu_watts") in ["gpu_watts"]:
                        ...
                        getattr(self, "display_mode", "cpu_watts")
                        == "alternating_watts"
                    ):
                ...
                for j in range(len(stops) - 1):
                    if stops[j]["value"] <= metric_value < stops[j + 1]["value"]:
                        start_stop = stops[j]
                        end_stop = stops[j + 1]
                        factor = (metric_value - start_stop["value"]) / (
                            end_stop["value"] - start_stop["value"]
                        )
                        colors.append(
                            interpolate_color(
                                start_stop["color"], end_stop["color"], factor
                            )
                        )
                        break
            else:
    ...
    def update(self):
        ...
        if self.config:
            ...
            self.temp_unit = {
                device: self.config.get(f"{device}_temperature_unit", "celsius")
            ...
            self.cycle_duration = int(
                self.config.get("cycle_duration", 5) / self.update_interval
            )
            ...
            self.metrics.update_interval = self.config.get(
            ...
            if self.display_mode not in display_modes:
                print(
                    f"Warning: Display mode {self.display_mode} not compatible, switching to cpu_watts."
                )
